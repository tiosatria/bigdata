...
def main():
    ...
    parser.add_argument('--source', type=str, help='source domain to clean. '
                                                   'if you do not fill this, the file name will then be used to determine which domain it comes from. file name convention sample: example_com.jsonl. if no source were found, cleaners params will then be used')
    ...
    parser.add_argument('--cleaners', nargs='+', help='List of cleaners to apply.', choices=['html'])
        ...
        try:
            ...
            if cfg is None:
                msg = (f"Could not infer domain config from filename '{input_path.name}'. "
                       f"default domain config will be used.")
                print(msg, file=sys.stderr)
                cfg = DomainConfigRegistry
            else:
